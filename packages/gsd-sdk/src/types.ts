...
// ─── Bridge-AI provider types (non-breaking extensions) ─────────────────────

/**
 * Known provider identifiers supported by bridge-ai.
 * Extensible — custom providers can use any string.
 */
...
/**
 * Provider configuration stored in project settings.
 */
export interface ProviderConfig {
  /** Provider identifier. */
  providerId: KnownProviderId | string;
  /** Model to use with this provider. */
  model?: string;
  /** Provider-specific base URL override. */
  baseUrl?: string;
  /** KSM secret name for the API key. */
...
/**
 * Bridge-AI execution context passed to session runners.
 * Extends GSD concepts with multi-provider tracking.
 */
export interface BridgeExecutionContext {
  /** Project ID in bridge-ai database. */
  projectId: string;
  /** Plan ID in bridge-ai database. */
  planId?: string;
  /** Phase ID in bridge-ai database. */
  phaseId?: string;
